...
import { BaseAccountMapper } from '../mappers/BaseAccountMapper';
...
export class BaseAccountApi extends BaseApi {
  create: (account: Account, cart: Cart | undefined) => Promise<Account> = async (
    ...
    account = await this.requestBuilder()
      ...
      .execute()
      .then((response) => {
        return BaseAccountMapper.commercetoolsCustomerToAccount(response.body.customer, locale);
      })
      .catch((error) => {
        if (error.code === 400) {
  ...
  addAddress: (account: Account, address: Address) => Promise<Account> = async (account: Account, address: Address) => {
    ...
    let addressData = BaseAccountMapper.addressToCommercetoolsAddress(address);
